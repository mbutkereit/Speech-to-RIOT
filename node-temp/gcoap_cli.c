...
static const coap_resource_t _resources[] = {
    ...
    { "/temperature", COAP_GET, _temp_handler },
};
...
/*
 * Server callback for /temperature. Returns the count of packets sent by the
 * CLI.
 */
static ssize_t _temp_handler(coap_pkt_t* pdu, uint8_t *buf, size_t len)
{
    ...
    char test[50];
    snprintf(test,50,"[{\"temperature\": %d }]",temperature_value);
    size_t payload_len = strlen(test);
    memcpy(pdu->payload, test, payload_len);
